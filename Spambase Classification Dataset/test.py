...
dense1 = tflearn.fully_connected(input, 70, activation='relu', 
								bias=True, weights_init='truncated_normal', bias_init='zeros', 
								regularizer='L2', weight_decay=0.001)
dropout1 = tflearn.dropout(dense1, keep_prob=0.5)

dense2 = tflearn.fully_connected(dropout1, 40, activation='relu', 
								bias=True, weights_init='truncated_normal', bias_init='zeros', 
								regularizer='L2', weight_decay=0.001)
...
model = tflearn.DNN(net, 
					...
					checkpoint_path='checkpoints/final-model/checkpoints', best_checkpoint_path='best_checkpoints/final-model/best_checkpoint')

# Start training (apply gradient descent algorithm)
model.fit(train_data, train_labels, 
			n_epoch=100, batch_size=None, validation_set=(test_data, test_labels), 
			show_metric=True, snapshot_step=100, run_id="final-model_2layer-70-40_relu_adam_online_lr-0.005")

# # Load best model
# with open('best_checkpoints/only-test/2layer-50-30_relu_adam_online_lr-0.002/checkpoint', 'rb') as file:
# 	best_model = file.read()[24:43]
# model.load('best_checkpoints/only-test/2layer-50-30_relu_adam_online_lr-0.002/'+best_model)
